...
class UserRoutes:
    def __init__(self):
        ...
        self.router.add_api_route(
            "/", self.read_users, methods=["GET"], response_model=list[UserInDB]
        )
        self.router.add_api_route("/{user_id}", self.delete_user, methods=["DELETE"])
        self.router.add_api_route(
            "/create", self.create_user, methods=["POST"], response_model=UserInDB
        )
        self.router.add_api_route(
            "/{user_id}", self.read_user, methods=["GET"], response_model=UserInDB
        )
        self.router.add_api_route(
            "/email/{email}",
            self.read_user_by_email,
            methods=["GET"],
            response_model=UserInDB,
        )
        self.router.add_api_route(
            "/{user_id}", self.update_user, methods=["PUT"], response_model=UserInDB
        )
    ...
    async def create_user(
        self, user: UserCreate, user_ops: UserOperations = Depends(get_user_ops), admin=Depends(require_admin)
    ):
        user.password = create_password_hash(user.password)
        return user_ops.create_user(user=user)

    async def read_user(
        self, user_id: int, user_ops: UserOperations = Depends(get_user_ops), admin=Depends(require_admin)
    ):
    ...
    async def read_user_by_email(
        self, email: str, user_ops: UserOperations = Depends(get_user_ops), admin=Depends(require_admin)
    ):
